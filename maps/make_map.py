...
def gazebo_world_box(fout, label, box, box_size, color):
    ...
    fout.write("        <pose> "+f"{box[0]:.4f} {box[1]:.4f} 0.15 0 0 {box[2]:.4f} </pose>\n")
    fout.write("        <collision name=\""+label+"_collision\">\n")
    fout.write("          <geometry>\n")
    fout.write("            <box>\n")
    fout.write("              <size>"+f"{box_size[0]:.4f} {box_size[1]:.4f} 0.3</size>\n")
    fout.write("            </box>\n")
    fout.write("          </geometry>\n")
...
def make_map(filename,
    ...
    print(" Grid map: ", grid_map.shape, grid_map.dtype)
    try:
        gray_map = cv2.cvtColor(grid_map, cv2.COLOR_BGR2GRAY)
        print(" gray map: ", gray_map.shape, gray_map.dtype)
        cv2.imwrite(filename+".pgm", gray_map)
    except Exception as e:
        print(" Exception in pgm write", e)
    # draw axes
